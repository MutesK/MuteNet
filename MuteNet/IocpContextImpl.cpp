...
namespace EventLoop
{
    SocketPtr IocpContextImpl::CreateSocket ( socket_t Socket )
    {
	    ...
	    CreateIoCompletionPort( reinterpret_cast<HANDLE>(Socket), _IocpHandle, reinterpret_cast<ULONG_PTR>(Ptr.get()), 0 );
	    return Ptr;
    }
		...
		const static auto WorkerRun = [&]()
		{
			...
			while (!IsStop())
			{
				...
				if (!GetQueuedCompletionStatus(_IocpHandle, &byteTransferred,
				                               reinterpret_cast<PULONG_PTR>(&WinSocketDescriptor), &lpOverlapped, _Timeout))
				{
	...
	void *IocpContextImpl::Extension::GetExtension ( socket_t socket, const GUID *FunctorPtr )
	{
		...
		auto ret = std::invoke(WSAIoctl, socket, SIO_GET_EXTENSION_FUNCTION_POINTER,
		                       (GUID*)FunctorPtr, sizeof(*FunctorPtr), &ptr, sizeof(ptr), &bytes, nullptr, nullptr);
